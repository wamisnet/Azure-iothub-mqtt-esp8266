...
GeneralFunction EspAzureIoTHub::az;

void EspAzureIoTHub::callback(char *topic, byte *payload, unsigned int length)
{
	String _azuredata;
	for (int i = 0; i < length; i++)
		_azuredata += (char)payload[i];
	az(_azuredata);
}

void EspAzureIoTHub::begin(String cs)
{
...
const char *EspAzureIoTHub::GetStringValue(String value)
{
...
String EspAzureIoTHub::splitStringByIndex(String data, char separator, int index)
{
	int found = 0;
	int strIndex[] = {0, -1};
...
String EspAzureIoTHub::urlEncode(const char *msg)
{
...
String EspAzureIoTHub::createIotHubSas(char *key, String url)
{
